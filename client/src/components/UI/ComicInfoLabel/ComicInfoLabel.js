...
export default class ComicInfoLabel extends Component {
	render() {
		...
		return <div className="comic-info-label">
			...
			<p className={this.props.className || ''}>Panels by {this.props.comic.group ? <span>members of <Link to={Util.route.group(this.props.comic.group.groupId)}>{this.props.comic.group.name}</Link>: </span> : null}{authors.map((author, idx) => {
			...
			{this.props.comic.challenge ? <p className={this.props.className || ''}>Challenge: <b>{this.props.comic.challenge}</b>.</p> : null}
		</div>
	}
}